...
拡張機能は常に読み込まれます。`PI_INPUT_LOCK=1` は起動時にロックを有効にするための設定で、未設定でも利用できます（起動時は無効です）。
...
起動時からロックを有効にする場合は、`PI_INPUT_LOCK=1` を設定します（任意です）。

```bash
export PI_INPUT_LOCK=1
```

`PI_INPUT_LOCK` を設定しなくても拡張機能は読み込まれます。実行中は次のコマンドで切り替えできます。
